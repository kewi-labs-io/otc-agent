...
export async function POST(request: NextRequest) {
  ...
  // FAIL-FAST: If pumpSwapPool exists, it must have all required fields
  if (pumpSwapPool) {
    if (pumpSwapPool.protocol !== "PumpSwap") {
      throw new Error(
        `Expected PumpSwap pool but got ${pumpSwapPool.protocol}`,
      );
    }
    if (!pumpSwapPool.solVault || !pumpSwapPool.tokenVault) {
      throw new Error(
        `PumpSwap pool missing solVault or tokenVault - invalid pool data`,
      );
    }

    // Configure pool on-chain
    console.log(
      `[Price Update] Using PumpSwap pool: ${pumpSwapPool.address}, price: $${pumpSwapPool.priceUsd}`,
    );
    ...
    const expectedPoolAddress = new PublicKey(pumpSwapPool.address);
    ...
    if (
      ...
      console.log(
        `[Price Update] Token pool config mismatch - current: ${currentPoolAddress.toBase58()} (type ${currentPoolType}), expected: ${pumpSwapPool.address} (type 3)`,
      );
    ...
    const tx = await program.methods
      ...
      .accounts({
        tokenRegistry: registryPda,
        bondingCurve: new PublicKey(pumpSwapPool.address),
        solVault: new PublicKey(pumpSwapPool.solVault),
        tokenVault: new PublicKey(pumpSwapPool.tokenVault),
      })
      .preInstructions([
