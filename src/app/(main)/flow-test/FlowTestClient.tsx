...
export default function FlowTestClient() {
  ...
  const executeCheckBalance = useCallback(async () => {
    ...
    if (testState.chain === "evm") {
      ...
      const balance =
        Number(accountInfo.value.amount) / Math.pow(10, tokenConfig.decimals);
      const requiredTokensWithFloor =
        requiredTokens > BigInt(100) ? requiredTokens : BigInt(100);
      const requiredReadable =
        Number(requiredTokensWithFloor) / Math.pow(10, tokenConfig.decimals);
  ...
  const executeApprove = useCallback(async () => {
    ...
    const formattedAmount =
      Number(amountToApprove) / Math.pow(10, tokenConfig.decimals);
  ...
  const executeDeposit = useCallback(async () => {
    ...
    if (testState.chain === "evm") {
      ...
      const formattedAmount =
        Number(amount) / Math.pow(10, tokenConfig.decimals);
      addLog(
      ...
      addLog(
        `Note: If token is not registered, this will attempt auto-registration (requires ~0.005 ETH + gas)`,
      );
      ...
      addLog(
        `Depositing ${Number(depositAmountLamports) / Math.pow(10, tokenConfig.decimals)} ${tokenConfig.symbol} to satisfy minimum order`,
      );
  ...
  const executeBuy = useCallback(async () => {
    ...
    if (testState.chain === "evm") {
      ...
      const buyAmountReadable =
        Number(buyAmountLamports) / Math.pow(10, tokenConfig.decimals);
      ...
      if (solanaDepositAmount < buyAmountLamports) {
        throw new Error(
          `Deposit (${Number(solanaDepositAmount) / Math.pow(10, tokenConfig.decimals)} tokens) is less than buy amount (${buyAmountReadable} tokens)`,
        );
      }
