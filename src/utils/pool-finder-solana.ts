...
async function findPumpFunBondingCurve(
  ...
  const tokenReserves =
    Number(virtualTokenReserves) / Math.pow(10, tokenDecimals);
  ...
  if (!priceRes.ok) {
    throw new Error(`CoinGecko SOL price fetch failed: ${priceRes.status}`);
  }
  const priceData = (await priceRes.json()) as { solana?: { usd?: number } };
  if (!priceData.solana) {
    throw new Error("CoinGecko SOL price data missing solana field");
  }
  if (priceData.solana.usd === undefined) {
    throw new Error("CoinGecko SOL price data missing usd field");
  }
...
async function findPumpSwapPools(
  ...
  for (const account of all) {
    ...
    if (baseToken && otherMint && otherMint.equals(mint)) {
      ...
      // Fetch real SOL price for accurate calculations
      const priceRes = await fetch(
        "https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd",
...
async function findRaydiumPools(
  ...
  for (const account of all) {
    ...
    if (baseToken && otherMint && otherMint.equals(mint)) {
      ...
      // Fetch real SOL price for TVL and price calculations
      const priceRes = await fetch(
        "https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd",
        { signal: AbortSignal.timeout(3000) },
      );
...
async function findMeteoraPools(
  ...
  // Fetch SOL price for TVL/price calculations
  const priceRes = await fetch(
    "https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd",
    { signal: AbortSignal.timeout(3000) },
  );
