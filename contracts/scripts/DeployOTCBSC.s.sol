...
/// @title DeployOTCBSC
/// @notice Deploy token-agnostic OTC contract and RegistrationHelper to BSC mainnet
contract DeployOTCBSC is Script {
    ...
    function run() external {
        ...
        console.log("Deploying to BSC Mainnet from:", deployer);
        ...
        OTC otc = new OTC(
            owner,
            IERC20(USDC),
            AggregatorV3Interface(BNB_USD_FEED), // Use BNB/USD for native currency pricing
            agent
        );
        ...
        string memory deploymentJson = string.concat(
            ...
            '    "bnbUsdFeed": "', vm.toString(BNB_USD_FEED), '"\n',
            '  }\n',
            '}'
        );
        vm.writeFile("deployments/bsc-mainnet.json", deploymentJson);
        console.log("Wrote deployment to deployments/bsc-mainnet.json");

        console.log("");
        console.log("=== Environment Variables ===");
        console.log("NEXT_PUBLIC_BSC_OTC_ADDRESS=%s", address(otc));
        console.log("NEXT_PUBLIC_BSC_REGISTRATION_HELPER_ADDRESS=%s", address(helper));
    }
}
