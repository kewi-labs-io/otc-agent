...
import { Connection, PublicKey, Keypair, ComputeBudgetProgram, Transaction, VersionedTransaction } from "@solana/web3.js";
...
async function main() {
  ...
  if (!deskOwner.equals(keypair.publicKey)) {
    throw new Error(`Signer is NOT the desk owner! Expected: ${deskOwner.toBase58()}, Got: ${keypair.publicKey.toBase58()}`);
  }
  ...
  for (const token of solanaTokens) {
    ...
    const currentPoolAddress = new PublicKey(registryInfo.data.subarray(POOL_ADDRESS_OFFSET, POOL_ADDRESS_OFFSET + 32));
    const currentPoolType = registryInfo.data[POOL_TYPE_OFFSET];
    
    console.log(`  Current: pool_type=${currentPoolType}, pool_address=${currentPoolAddress.toBase58().slice(0, 8)}...`);
    ...
    const poolType = pool.protocol === "PumpSwap" ? 3 
                   : pool.protocol === "Raydium" ? 1 
                   : pool.protocol === "Orca" ? 2 
                   : 0;
    const poolAddress = new PublicKey(pool.address);

    const priceDisplay = pool.priceUsd !== undefined ? `$${pool.priceUsd.toFixed(8)}` : "price unavailable";
    console.log(`  Found: ${pool.protocol} pool at ${pool.address.slice(0, 8)}..., price: ${priceDisplay}`);
