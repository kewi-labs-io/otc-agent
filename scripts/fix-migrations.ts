...
async function main() {
  ...
  try {
    ...
    if (migrationSchemaResult.rows.length > 0) {
      ...
      await client.query(`DELETE FROM migrations._migrations WHERE plugin_name = '@elizaos/plugin-sql'`);
      await client.query(`DELETE FROM migrations._journal WHERE plugin_name = '@elizaos/plugin-sql'`);
      await client.query(`DELETE FROM migrations._snapshots WHERE plugin_name = '@elizaos/plugin-sql'`);
