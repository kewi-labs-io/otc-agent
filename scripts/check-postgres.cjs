...
const DEV_PORT = process.env.POSTGRES_DEV_PORT ? Number(process.env.POSTGRES_DEV_PORT) : 5439;
...
async function main() {
  const hasPostgresUrl = !!process.env.POSTGRES_URL || !!process.env.POSTGRES_DATABASE_URL;
  ...
  for (let i = 0; i < 20; i++) {
    if (await isPortOpen(DEV_PORT)) {
      ...
      try {
        execSync(`docker exec ${CONTAINER_NAME} psql -U postgres -d eliza -c "CREATE EXTENSION IF NOT EXISTS vector;"`, { stdio: 'ignore' });
      } catch {}
      return;
    }
